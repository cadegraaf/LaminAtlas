...
#
# Modified by Carolyn 3/2/2020
# Will now take manual alignments from pair to rData. 
#
...
my $usage = "usage: nimblescanWrap.pl -input inputfiles -designfile designfile -output outputdir [-localalign] [-dyeswap] [-verbose]\n";

## Options:
my (@input, $designfile, $output) = ((),(),());
GetOptions('input=s{,}'     => \@input,
           ...
           'verbose'         => \$VERBOSE)
           'dyename'         => \$dyename;
(@input and defined($designfile) and defined($output)) or die $usage;

## Check options
if(scalar(@input)<1) {
  die "'input' argument incorrect";
}
...
#some files are named Cy3 / Cy5, others have the emmision frequency of the dye
#the dye name is default, but if dyname flag = freq change this
...
foreach my $in (@input) {
  die "$in not a TIFF image (ie no '.tif' extension)"
    unless $in =~ m/(?:.*\/)*(.*)\.tif/;
  ...
  my $command = "$TIFFSPLIT $in $output$imagebasename";
  ...
  print "Rotating and flipping $in\n";
...
dat <- data.frame(seqname = merged\$CHROMOSOME,                 start = merged\$POSITION, 
                  end     = merged\$POSITION+49,  score = merged\$M, 
                  A       = merged\$A,                          probe = merged\$PROBE_ID);

#hard coded that length is 50 as not in the pos file that I have
#dat <- data.frame(seqname = merged\$CHROMOSOME,                 start = merged\$POSITION, 
